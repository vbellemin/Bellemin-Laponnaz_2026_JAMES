...
Bellemin-Laponnaz, V., Le Guillou, F., Ubelmann, C., Blayo, E., & Cosme, E. (2026). **A variational method for reconstructing and separating balanced motions and internal tides from wide-swath altimetric sea surface height observations**. *Journal of Advances in Modeling Earth Systems*, 18, e2025MS005382.
...
This repository provides the code required to create the **OSSE experiment** ([`./OSSE_generator/`](./OSSE_generator)), reproduce the **SSH reconstruction experiments** ([`./mapping/`](./mapping)) and compute the **performance analyses** ([`./analysis/`](./analysis)) presented in the manuscript.
...
2. **Create a new environment**
   ```bash
   conda create -n env_bl_james python=3.10
   ```
   ```bash
   conda activate env_bl_james
   ```
...
### Creation of the OSSE experiment ([`./OSSE_generator/`](./OSSE_generator))
...
The notebooks used to perform the filtering operations are listed below. All notebooks are located in the [`./OSSE_generator/`](./OSSE_generator) directory.

* [`extract_bm.ipynb`](./OSSE_generator/extract_bm.ipynb) — Applies a temporal low-pass filter to the ground-truth total SSH to extract the **balanced motions** (reference field).
* [`extract_bar.ipynb`](./OSSE_generator/extract_bar.ipynb) — Applies a spatial low-pass filter to the high-frequency SSH to extract the **barotropic tide** (used for correction).
* [`extract_it.ipynb`](./OSSE_generator/extract_it.ipynb) — Applies a temporal band-pass filter to the internal gravity wave field to extract the **semi-diurnal total internal tide**.
* [`extract_modes.ipynb`](./OSSE_generator/extract_modes.ipynb) — Applies a spatial band-pass filter to the semi-diurnal internal tide field to isolate the **mode-1 semi-diurnal internal tides** (reference field).
* [`notebook_miost_like_ssh.ipynb`](./OSSE_generator/notebook_miost_like_ssh.ipynb) - Applies a temporal and spatial lowpass filter to the reference balanced motions to mimick the resolution of **operational mapping products** (used for initial and boundary conditions of the QG model). 

Synthetic observations are generated by performing a trilinear interpolation along the satellite tracks. The corresponding notebooks are located in the [`./OSSE_generator/interp_satellite_track`](./OSSE_generator/interp_satellite_track) directory:

* [`interp_nadir.ipynb`](./OSSE_generator/interp_satellite_track/interp_nadir.ipynb) for the Nadir satellites.
* [`interp_swot.ipynb`](./OSSE_generator/interp_satellite_track/interp_swot.ipynb) for the SWOT satellite in the CalVal orbit.

### SSH reconstruction ([`./mapping/`](./mapping))
...
[`./mapping/notebooks/notebook.ipynb`](./mapping/notebooks/notebook.ipynb)
...
[`./mapping/config_files/`](./mapping/config_files/)

* [`config_QGSW.py`](./mapping/config_files/config_QGSW.py) — Framework combining **quasi-geostrophic (QG)** and **shallow-water (SW)** models for the reconstruction and separation of **balanced motions** and **internal tides**.

* [`config_QG.py`](./mapping/config_files/config_QG.py) *(sensitivity experiment)* — Framework based on a **QG model only**, used to reconstruct **balanced motions**.

* [`config_QGSW_notime.py`](./mapping/config_files/config_QGSW_notime.py) *(sensitivity experiment)* — Framework combining **QG** and **SW** models, where the time dependency of the SW control variables is removed. This configuration is used to reconstruct **balanced motions** and the **coherent part of the internal tides**.
...
* [`fig03_05_07_spectral_analysis.ipynb`](./analysis/fig03_05_07_spectral_analysis.ipynb) — Spectral analysis (Power Spectral Density, PSD, and associated PSD score). Generates Figures **03**, **05**, and **07**.
