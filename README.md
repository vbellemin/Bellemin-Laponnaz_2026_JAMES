...
Synthetic observations are generated by performing a trilinear interpolation along the satellite tracks. The corresponding notebooks are available in `interp_nadir.ipynb` for the Nadir satellites and `interp_swot.ipynb` for the SWOT satellite in the CalVal orbit.
